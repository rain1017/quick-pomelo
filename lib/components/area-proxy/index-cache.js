...
proto.get = function(areaId){
	...
	if(this.index[areaId]){
		...
		promise = Q.fcall(function(){
			return self.areaBackend.getAreaOwnerId(areaId);
...
proto._saveCache = function(areaId, serverId){
	...
	this.areaBackend.on(util.format('area:%s:update', areaId), cache.onUpdate);
	this.areaBackend.on(util.format('area:%s:remove', areaId), cache.onRemove);
...
proto._deleteCache = function(areaId){
	...
	this.areaBackend.removeListener(util.format('area:%s:update', areaId), cache.onUpdate);
	this.areaBackend.removeListener(util.format('area:%s:remove', areaId), cache.onRemove);
