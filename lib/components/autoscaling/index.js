...
proto.start = function(cb){
	...
	this.app.areaBackend.on('area:create', this.onCreateArea);
...
proto.stop = function(force, cb){
	...
	this.app.areaBackend.removeListener('area:create', this.onCreateArea);
...
proto.autoAssignAreas = function(){
	var areaBackend = this.app.areaBackend;
	var self = this;

	return Q.fcall(function(){
		return areaBackend.getAreasNotInServers(Object.keys(self.servers), self.assignLimit);
	}).then(function(areas){
		return Q.allSettled(areas.map(function(area){
			return Q.fcall(function(){
				//Area assigned to one server that not online
				if(area._server){
					return areaBackend.releaseAreaForce(area._id).then(function(){
...
proto.autoReleaseAreas = function(){
	...
	return Q.allSettled(Object.keys(self.servers).map(function(serverId){
		...
		return Q.fcall(function(){
			return self.app.areaBackend.getAcquiredAreaIds(serverId, self.releaseLimit);
		}).then(function(areaIds){
			return Q.allSettled(areaIds.map(function(areaId){
				return self.app.areaProxy.quitServer(areaId).then(function(){
...
proto.assignArea = function(areaId){
	...
	var self = this;
	return Q.fcall(function(){
		return self.app.areaProxy.joinServer(areaId, serverId);
	}).then(function(){
