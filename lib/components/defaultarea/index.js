 ...
 *
 * Tips:
 * Call playerProxy.invokePlayer when to operate the single player, otherwise call areaProxy.invokeArea
...
proto.onStart = function(){
	...
	this.freeUpdateInterval = setInterval(function(){
		...
		self.app.areaBackend.updateArea(self, {free : free}).catch(function(e){
			logger.warn(e.stack);
		});
	}, this.freeUpdate);
};
